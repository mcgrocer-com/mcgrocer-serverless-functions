...
/**
 * Worker endpoint for batch publishing draft products.
 * Called by the cron job and self-chains until all qualifying products are published.
 */
async function handler(req, res) {
  ...
  try {
    ...
    if (hasMore) {
      ...
      const nextUrl = `${baseUrl}/api/publish-batch?chain=${chain + 1}`;
